...
export default function ExpiredCalendarPage() {
    ...
    fetch("/api/unit-produksi")
      ...
      .then(data => {
        const grouped = (data || []).reduce((acc: any, curr: any) => {
          ...
          if (curr.siteArea && !acc[curr.namaRegional].includes(curr.siteArea)) acc[curr.namaRegional].push(curr.siteArea);
          return acc;
        }, {});
  ...
  useEffect(() => { fetchPOData(); }, [fetchPOData]);
  ...
  if (isLoading || role === null) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <p className="text-sm font-semibold text-slate-500">Memuat data kalender expired...</p>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1.5">
            ...
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Expired Calendar</h1>
          </div>
          <p className="text-slate-500 text-sm ml-[52px]">Visualisasi tanggal kadaluarsa (expired) PO untuk monitoring limit waktu.</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-2">
          {role !== "sitearea" && (
            <div className="space-y-1.5">
              ...
              <SmoothSelect
                ...
                onChange={(v) => { setSelectedRegional(v); setSelectedSiteArea("ALL"); setSelectedDateKey(null); }}
                options={[{ value: "ALL", label: "Semua Regional" }, ...Object.keys(regionalData).sort().map(r => ({ value: r, label: r }))]}
                disabled={role !== "pusat"}
              />
            </div>
          )}
          <div className="space-y-1.5">
            ...
            <SmoothSelect
              ...
              options={[{ value: "ALL", label: "Semua Site Area" }, ...(regionalData[selectedRegional] || []).map(s => ({ value: s, label: s }))]}
          ...
          <div className="space-y-1.5">
            ...
            <SmoothSelect value={selectedMonth} onChange={(v) => { setSelectedMonth(v); setSelectedDateKey(null); }} options={MONTH_NAMES.map((m, i) => ({ value: String(i + 1), label: m }))} />
          </div>
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
              <CalendarRange size={12} /> Tahun
            </label>
            <SmoothSelect value={selectedYear} onChange={(v) => { setSelectedYear(v); setSelectedDateKey(null); }} options={YEARS.map(y => ({ value: String(y), label: String(y) }))} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <button onClick={() => { 
            ...
            if (m < 1) { m = 12; y--; }
            setSelectedMonth(String(m)); setSelectedYear(String(y)); setSelectedDateKey(null);
          }} className="p-2 rounded-xl hover:bg-slate-100 text-slate-500 transition-colors"><ChevronLeft size={20} /></button>
          
          <div className="text-center">
            <h3 className="text-lg font-bold text-slate-800">{MONTH_NAMES[month - 1]} {year}</h3>
            {selectedSiteArea && <p className="text-xs text-slate-400 font-medium mt-0.5">{selectedSiteArea} · {selectedRegional}</p>}
          </div>
          ...
          <button onClick={() => {
            ...
            if (m > 12) { m = 1; y++; }
            setSelectedMonth(String(m)); setSelectedYear(String(y)); setSelectedDateKey(null);
          }} className="p-2 rounded-xl hover:bg-slate-100 text-slate-500 transition-colors"><ChevronRight size={20} /></button>
        </div>

        <div className="grid grid-cols-7 border-b border-slate-100">
          {DAY_LABELS.map(d => <div key={d} className="text-center py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">{d}</div>)}
        </div>
          ...
          <div className="border border-slate-200 rounded-xl bg-white shadow-sm overflow-hidden m-6">
            <div className="grid grid-cols-7 bg-slate-50 border-b border-slate-200">
              {DAY_LABELS.map((day, i) => (
                <div key={i} className="py-2 text-center text-[10px] font-black text-slate-500 uppercase tracking-wider">{day}</div>
              ))}
            </div>
            <div className="grid grid-cols-7 gap-px bg-slate-200">
              {calendarCells.map((day, idx) => {
                if (day === null) return <div key={`empty-${idx}`} className="bg-slate-50/50 min-h-[50px] sm:min-h-[60px]" />;
                ...
                return (
                  <div key={dateKey} onClick={() => setSelectedDateKey(dateKey)} className={`relative bg-white min-h-[50px] sm:min-h-[60px] p-2 cursor-pointer transition-all hover:bg-slate-50 group ${hasPOs ? "bg-rose-50/60 hover:bg-rose-100/60" : ""} ${isSelected ? "ring-2 ring-inset ring-rose-500 z-10" : ""}`}>
                    <div className="flex justify-between items-start">
                      <span className={`text-[11px] font-bold w-5 h-5 flex items-center justify-center rounded-full ${today ? "bg-rose-600 text-white shadow-sm" : hasPOs ? "text-rose-900" : "text-slate-500"}`}>
                        {day}
                      </span>
                      {hasPOs && (
      ...
      {!selectedSiteArea && selectedRegional !== "ALL" && (
        <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-3xl border border-dashed border-slate-200">
          <div className="p-4 bg-slate-50 rounded-2xl mb-4"><AlertTriangle size={32} className="text-slate-300" /></div>
          <p className="text-sm font-bold text-slate-500">Pilih Site Area untuk memantau data expired</p>
        </div>
      )}
    </div>
  );
