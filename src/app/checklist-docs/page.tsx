...
import { Eye, Save, X, Pencil, Building2, FileText, CheckCircle2, ChevronRight, MapPin } from "lucide-react";
import {
  ColumnDef,
  getCoreRowModel,
  useReactTable,
...
export default function ChecklistDocsPage() {
  ...
  const columns = useMemo<ColumnDef<Row>[]>(() => [
    ...
    { header: "REGIONAL", id: "regional", accessorKey: "regional", cell: ({ row }) => { const reg = row.original.regional || row.original?.UnitProduksi?.namaRegional || "-"; return <div className="flex items-center gap-2 whitespace-nowrap"><MapPin size={16} className="text-rose-500 shrink-0" /><span className="text-slate-700 dark:text-slate-300 font-semibold">{reg}</span></div>; } },
