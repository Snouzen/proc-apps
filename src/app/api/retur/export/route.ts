...
export async function GET(request: Request) {
  try {
    ...
    worksheet.columns = [
      ...
      { header: "NAMA COMPANY", key: "namaCompany", width: 30 },
    ...
    data.forEach((item, index) => {
      worksheet.addRow({
        ...
        toko: item.namaCompany || "-",
        namaCompany: item.RitelModern?.namaPt || "-",
        inisial: item.inisial || "-",
        link: item.link || "-",
        produk: item.produk || "-",
        qtyReturn: Number(item.qtyReturn || 0),
        kg: Number(item.qtyReturn || 0) * (item.Product?.satuanKg || 1),
