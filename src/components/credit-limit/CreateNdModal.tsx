...
export function CreateNdModal({ open, onClose, batchCode, pos }: CreateNdModalProps) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      await generateNdWord(formData, pos);
    } catch (err: any) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-gradient-to-br from-slate-50 via-white to-amber-50/30 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 rounded-2xl w-full max-w-[1800px] h-[95vh] shadow-2xl relative flex flex-col overflow-hidden border border-slate-200 dark:border-slate-700">
        
        {/* Header */}
        <div className="shrink-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/60 dark:border-slate-700/60">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                    ...
                    {batchCode && (
                      <span className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-black uppercase tracking-wider">
                        Batch {batchCode}
                      </span>
                    )}
                  </h1>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {isGeneratingPdf ? "Generating..." : "Cetak PDF"}
                </button>
        ...
        <div className="flex-1 overflow-hidden">
          <div className="flex flex-col lg:flex-row h-full">
            ...
            {/* Right: Form */}
            <div
              className={`lg:w-[460px] xl:w-[500px] lg:flex flex-col ${!previewMode ? "flex" : "hidden"} overflow-auto h-full border-l border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-900/30`}
            >
          ...
          <li>
            ...
            {pos && pos.length > 0 ? (
              (() => {
                ...
                return (
                  <div className="mt-2 my-2 overflow-x-auto">
                    <table className="w-full border-collapse border border-slate-800 text-[9px]" style={{ tableLayout: "auto" }}>
                      <thead>
                        <tr className="bg-[#daeef3]">
                          <th className="border border-slate-800 p-1 font-semibold text-center w-[30px]">No.</th>
                          <th className="border border-slate-800 p-1 font-semibold text-center">Infrastruktur</th>
                          <th className="border border-slate-800 p-1 font-semibold text-center">Id Pelanggan</th>
                          <th className="border border-slate-800 p-1 font-semibold text-center">Nama Pelanggan</th>
                          <th className="border border-slate-800 p-1 font-semibold text-center">No PO</th>
                          <th className="border border-slate-800 p-1 font-semibold text-center">Nilai (Rp)</th>
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(groupedPos).map(([site, items]: [string, any], groupIndex) => {
                          const subtotal = items.reduce((sum: number, po: any) => sum + Number(po.totalNominal || po.nominal || 0), 0);
                          return (
                            <Fragment key={site}>
                              {items.map((po: any, idx: number) => (
                                <tr key={idx}>
                                  {idx === 0 && (
                                    <td rowSpan={items.length} className="border border-slate-800 p-1 bg-white font-bold align-middle text-center">{groupIndex + 1}</td>
                                  )}
                                  {idx === 0 && (
                                    <td rowSpan={items.length} className="border border-slate-800 p-1 bg-white font-bold align-middle uppercase text-center whitespace-nowrap">{site}</td>
                                  )}
                                  <td className="border border-slate-800 p-1 text-center whitespace-nowrap">{po.kodeVendor || "-"}</td>
                                  <td className="border border-slate-800 p-1 text-left">{po.RitelModern?.inisial || "-"}</td>
                                  <td className="border border-slate-800 p-1 text-center whitespace-nowrap">{po.noPo}</td>
                                  <td className="border border-slate-800 p-1 text-right whitespace-nowrap">
                                    {Number(po.totalNominal || po.nominal || 0).toLocaleString("id-ID")}
                                  </td>
                                </tr>
                              ))}
                              <tr>
                                <td colSpan={5} className="border border-slate-800 p-1 font-bold bg-[#f8f9fa] text-center uppercase">SUBTOTAL</td>
                                <td className="border border-slate-800 p-1 font-bold bg-[#f8f9fa] text-right whitespace-nowrap">{subtotal.toLocaleString("id-ID")}</td>
                              </tr>
                            </Fragment>
                          );
                        })}
              ...
              <div className="mt-2 text-slate-400 italic text-[10px]">
                {batchCode ? `[Data PO di Batch Credit Limit ${batchCode} - akan diintegrasikan]` : "[Data Batch Credit Limit - akan diintegrasikan]"}
              </div>
            )}
          </li>
        </ol>
